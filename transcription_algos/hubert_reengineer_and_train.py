...
raw_waveform_training_examples = raw_waveform_training_examples[40:50]
transcript_training_example_labels = transcript_training_example_labels[40:50]
...
def create_labels_and_pad():
    ...
    max_amount_of_labels_vals = max([len(i) for i in labels])
    
    for label_vector in labels:
        zeros = [0] * (max_amount_of_labels_vals - len(label_vector))
        label_vector.extend(zeros)
...
def run_hubert(inputs, labels, mask, epochs = 3):
    ...
    #processor = Wav2Vec2Processor.from_pretrained("facebook/hubert-large-ls960-ft")
    #model = HubertForCTC.from_pretrained("facebook/hubert-large-ls960-ft")
    inputs, validation_inputs, labels, validation_labels, mask, validation_mask = train_test_split(inputs, labels, mask, test_size = 0.2)
    ...
    for batch in range(epochs):
        inputs, labels, mask = shuffle(inputs, labels, mask)
        ...
        print('second',loss)
        #with torch.no_grad():
        validation_output = model(validation_inputs, labels = validation_labels, attention_mask = validation_mask)
...
lowest_loss_model, lowest_loss, loss_list  = run_hubert(raw_waveform_training_examples, labels, training_mask)
